...
class Admin extends controller
{
        ...
        if (!empty(Session::get('admin'))) {
            ...
            foreach ($users as $user) {
                ...
                if($utype == 1){
                    ...
                    $totalLeads = Attendance::where('user_id', $userId)
                ...
                $data['attendance'][$userId]['total_leads'] = $totalLeads;
                }
                // dd($data['attendance']);
            }
    ...
    {
        $data['user'] = Attendance::select('attendance.mark', 'attendance.mark_date', 'attendance.login_time', 'attendance.logout_time', 'attendance.nonpause', 'attendance.sale_made', 'attendance.customer', 'attendance.leads', 'user.name')->where('mark_date', $date)
            ->leftJoin('user', 'user.id', '=', 'attendance.user_id')->get();
        return response()->json($data);
    }
    // public function sales(Request $request)
    // {
    ...
    public function add_user(Request $request)
    {
            if (!empty(Session::get('admin'))) {
                ...
                if (!empty($request->post('_token'))) {
                    $selectedKeys = [
                        'name', 'gender', 'mobile', 'work_type', 'pf', 'joining_date', 'server_ip',
                    ...
                    if ($ext['user_type'] == 1) {
                        ...
                        if ($ch->res == "USER ADDED") {
                            ...
                            return redirect(env('APP_URL') . 'admin/users/create')->withErrors(['msg' => $ch]);
                        }
                    }
                    $ext['crm_id'] = $crm_id;
    ...
    public function experience(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            $user = $this->user($user_id);
            $data['ext'] = $ext = $request->post();
            return view('admin.users.exper', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    ...
    public function offup(Request $request)
    {
        if ($request->hasFile('pdfFile')) {
            ...
            $to = $request->email;
            Mail::send('mail', $data, function ($message)  use ($te, $to) {
                //   echo $te;exit;
                $message->to($to)->subject('Re: Offer letter');
                $message->cc('admin@pantheondigitals.com');
                $message->attach('/home/u991583824/domains/pantheondigitals.com/public_html/subdomain/hr/public/uploads/' . $te);
    ...
    public function filter(Request $request)
    {
        ...
        if (!$request->has('custome_filter')) {
            ...
            // Apply the asset_type filter if it's present in the request
...
// \DB::enableQueryLog();
            if ($request->has('custome_filter') && $request->input('custome_filter')) {
                ...
                ->leftJoin('user_assets', 'user_assets.assets_id', '=', 'assets.id')
                ->where(function ($q) use ($customFilter) {
                    $q->where('assets.serial_number', 'like', '%' . $customFilter . '%')
                        ->orWhere('user_assets.status', 'like', '%' . $customFilter . '%');
                })
                ->where(function ($q) {
                    $q->where('user_assets.from', '<=', '2024-04-02')
                        ->where(function ($q2) {
                            $q2->where('user_assets.to', '>=', '2024-04-02')
