...
class Admin extends controller
{
    public function login(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            return redirect(env('APP_URL') . 'admin/index');
        } else {
            if (!empty($request->post('_token'))) {
                $ext = $request->post();
                $user = Adminmodel::where('username', $ext['user_id'])->where('password', $ext['password'])->get();
                if ($user->count() == 1) {
                    $request->session()->put('admin', $user[0]->id);
                    return redirect(env('APP_URL') . 'admin/index');
                } else {
                    Session::flash('message', 'Your email or password is incorrect.');
    ...
    public function grants(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if ($request->_token) {
                ...
                // dd($ext);
                unset($ext['_token']);
                $data['permission_access'] = User_permission::create([
                    'uid' => $ext['uid'],
                    'permission' => $ext['permission_id']
                ]);
                if ($data['permission_access']) {

                    return redirect()->back()->with('success', 'succefully');
                } else {

                    return redirect()->back()->with('error', 'some error occured');
                }
            } else {
                $data['users'] = User::select('id', 'user_id', 'name')->where('status', 1)->get();
                $data['grants'] = Permissions::all();
                return view('admin.grants', $data);
            }
        }
    }
    ...
    public function attendance(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            $user = $this->user($user_id);
            $data['user'] = $user;
            return view('admin.users.attendance', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    ...
    public function sales(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if ($users->count() == 0) {
                $data['attendance'] = [];
                return view('admin.users.attendance.sales', $data);
            }
            ...
            $holidayCheck = Holiday::select('hdate')->whereBetween('hdate', [$firstDay, $lastDay])->where(function ($q) use ($request) {
                $q->where('user_type', $request->input('type'))->orWhere('user_type', 0);
            })->get();
    ...
    public function attendance_mark(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            return view('admin.users.mark', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    ...
    public function full_report($id, Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if ($profile->count() > 0) {
                ...
                $data['document'] = User_document::select('User_document.*')->where('uid', $id)->where('doc_verify', '<>', 2)->get();
                // $data['alluser'] = User::where('status',1)->get();
                $data['user_file'] = User_file::where('uid', $id)->get();
                return view('admin.users.report3', $data);
            } else {
                return redirect(env('APP_URL') . 'admin/users');
            }
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function doc_verify($id, Request $request)
    {
        if (!empty(Session::get('admin'))) {
            $user_id = Session::get('admin');
            $user = $this->user($user_id);
            ...
            if ($request->isMethod('POST')) {
                $ext = $request->post();
                // dd($ext);
                User_document::where('uid', $id)->where('id',$ext['id'])->where('doc_verify', [0, 1])->update([
                    'doc_verify' => $ext['doc_verify']
                ]);
                if($ext['doc_verify']==1){
                    Session::flash('message', 'Document verified');
                    Session::flash('alert-class', 'alert-success');
                }elseif($ext['doc_verify']==2){
                    Session::flash('message', 'Document rejected');
                    Session::flash('alert-class', 'alert-danger');
                }
                
                return  redirect(env('APP_URL') . 'admin/attendance/full_report/' . $id);
            }
        }
    }
    public function full_report2($id, Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if ($profile->count() > 0) {
                ...
                $data['user_file'] = User_file::where('uid', $id)->get();
                $data['use'] = $use;
                return view('admin.users.report2', $data);
            } else {
                return redirect(env('APP_URL') . 'admin/users');
            }
        } else {
    ...
    public function leave(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            return view('admin.users.leave', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    ...
    public function teams(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if ($request->isMethod('POST')) {
                ...
                return view('admin.users.team', $data);
            }
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
        ...
        if (!empty(Session::get('admin'))) {
            ...
            if (!empty($request->post('_token'))) {
                ...
                if ($ext['user_type'] == 1) {
                    ...
                    $ch = json_decode($ch);
                    if ($ch->res == "USER ADDED") {
                    } else {
                        $err = "dialer not created ";
                    }
                ...
                return redirect(env('APP_URL') . 'admin/users')->withErrors(['msg' => $err]);
            } else {
                $data['user'] = $user;
                $data['users'] = User::where('status', 1)->get();
    ...
    //             Holiday::create($ext);
    //             return redirect(env('APP_URL') . 'admin/holiday');
    //         } else {
    //             $data['user'] = $user;
    //             $data['holi'] = Holiday::all();
    //             return view('admin.users.holiday', $data);
    //         }
    //     } else {
    //         return redirect(env('APP_URL') . 'admin');
    //     }
    ...
    public function ticket(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            return view('admin.users.ticket', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function new_candidate(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if (!empty($request->post('_token'))) {
                ...
                $data['user'] = $user;
                return view('admin.users.new_candidate', $data);
            }
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    ...
    public function candidate(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if ($request->_token) {
                ...
                return view('admin.users.candidate', $data);
            }
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    ...
    public function create_offer(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            return view('admin.users.create_offer', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
            ...
            if ($request->_token) {
                ...
                if ($ext['by_user_type'] == 'osw') {
                    if (isset($ext['user_ids']) and !empty($ext['user_ids'])) {
                        ...
                        Session::flash('alert-class', 'alert-danger');
                        return redirect(env('APP_URL') . 'admin/announcement');
                    }
                }
    ...
    public function offer(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            return view('admin.users.offer', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function fnf(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            $data['salary_in_word'] = $result . "RUPEES  ";
            // ---------------------------------------------------------------------------------------------------------------------
            return view('admin.users.fnf', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function experience(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            $user = $this->user($user_id);
            $data['ext'] = $ext = $request->post();
            return view('admin.users.experience', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function confirmation(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            $user = $this->user($user_id);
            $data['ext'] = $ext = $request->post();
            return view('admin.users.confirmation', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function relieving(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            $user = $this->user($user_id);
            $data['ext'] = $ext = $request->post();
            return view('admin.users.relieving', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function appoint(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            $points = ($point) ?
                ...
                $words[$point = $point % 10] : '';
            $data['salary_in_word'] = $result . "RUPEES  ";
            return view('admin.users.appoint', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function increment(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            return view('admin.users.increment', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function appraisal(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            $user = $this->user($user_id);
            $data['ext'] = $ext = $request->post();
            return view('admin.users.appraisal', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    ...
    public function reject($id, Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            return redirect(env('APP_URL') . 'admin/onboard/candidate');
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    ...
    public function hired_candidate(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if ($request->_token) {
                ...
                return view('admin.users.rejected', $data);
            }
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    ...
    public function add_candidate(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if (!empty($request->post('_token'))) {
                ...
                return redirect(env('APP_URL') . 'admin/onboard/add_candidate');
            } else {
                $data['user'] = $user;
                return view('admin.users.add_candidate', $data);
            }
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    ...
    public function interview(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if ($request->_token) {
                ...
                return view('admin.users.interview', $data);
            }
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
        ...
        if (!empty(Session::get('admin'))) {
            ...
            if ($request->_token) {
                ...
                if (Assets::where('serial_number', $ext['serial_number'])->count() == 0) {
                    ...
                    return redirect(env('APP_URL') . 'admin/ass_ass')->with('success', 'serial_number already exists');
                }
                return redirect(env('APP_URL') . 'admin/ass_ass');
            } else {
                if (!empty($request->id)) {
                    $data['edit'] = $ass->get();
                    return view('admin.edit_ass', $data);
                } else {
                    ...
                    $data['alluser'] = User::where('status', 1)->get();
                    return view('admin.ass_ass', $data);
                }
            }
        } else {
            return redirect(env('APP_URL') . 'admin');
    ...
    //                     }
    //                     return redirect(config('app.url') . 'admin/ass_ass');
    //                 }
            ...
            if ($request->has('assorunass') && $request->input('assorunass')) {
                ...
                if ($status == 'assign') {
                    $query = Assets::select('assets.*', 'user_assets.status as assignment_status')
                        ->leftJoin('user_assets', 'user_assets.assets_id', '=', 'assets.id')
                        ->where('user_assets.status', '=', $status);
                } elseif ($status == 'unassignd') {
                    $today = date('Y-m-d');

                    $query = Assets::select('assets.*', 'user_assets.status as assignment_status')
                        ->leftJoin('user_assets', 'user_assets.assets_id', '=', 'assets.id')
                        ->leftJoinSub(
                            // Subquery to get the latest 'from' date for each 'assets_id'
                            User_assets::selectRaw('assets_id, MAX(`from`) as max_from')
                                ->where('status', '=', 'assign') // Consider only "assigned" statuses
                                ->groupBy('assets_id'),
                            'latest_assignments',
                            function ($join) {
                                $join->on('user_assets.assets_id', '=', 'latest_assignments.assets_id')
                                    ->on('user_assets.from', '=', 'latest_assignments.max_from');
                            }
                        )
                        ...
                        ->where(function ($query) use ($today) {
                            $query->where(function ($q) use ($today) {
                                // Covers assets whose assignment has ended
                                $q->where('user_assets.to', '<', $today)
                                    ->orWhereNull('user_assets.to');
                            })
                                ->orWhere(function ($q) {
                                    // Covers assets never assigned (no corresponding entry in 'user_assets')
                                    $q->whereNull('user_assets.assets_id');
                                });
                        })
                        ->groupBy('assets.id');
                }
    ...
    public function policies(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            if ($request->isMethod('post')) {
                ...
                return redirect(env('APP_URL') . 'admin/policies');
            } else {
    ...
    public function attendance2(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            $user = $this->user($user_id);
            $data['user'] = $user;
            return view('admin.users.attendance2', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
    public function sales2(Request $request)
    {
        if (!empty(Session::get('admin'))) {
            ...
            return view('admin.users.attendance.sales2', $data);
        } else {
            return redirect(env('APP_URL') . 'admin');
        }
    }
