            ...
            <div class="app-main__inner ">
                <div class="borderRadius overflow-hidden lReq baseShadow bg-white my-2 mx-auto p-2 " style="width: 98%;">
                    ...
                    <table class="table table-bordered text-center capitalize">
                        ...
                        <tr>
                            <td><a href="{{env('APP_URL').'adper/attendance/full_report/'.$leave->user_id}}"><p class="m-0">{{$leave->user_ida}}</p></a></td>
...
<script>
    ...
    $.ajax({
        url: '{{env('APP_URL')}}adper/leave_approve/' + leaveId,
        method: 'POST',
        data: {
    ...
    $.ajax({
        url: '{{env('APP_URL')}}adper/leave_reject/' + leaveId,
        method: 'POST',
        data: {
            response: responseValue,
            _token: '{{ csrf_token() }}'
