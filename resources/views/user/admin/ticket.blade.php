            ...
            <div class="app-main__inner ">
                <div class="borderRadius overflow-hidden lReq baseShadow bg-white my-2 mx-auto p-3 py-3 " style="width: 98%;">
                  ...
                  <table class="table table-bordered text-center">
                    ...
                    <tr> 
                        <td>
                           <a href="{{env('APP_URL')}}adper/attendance/full_report/{{$tic->uid}}">{{$tic->name}}</a> </td>
                        <td>{{date('d M, Y',strtotime($tic->created_at))}}</td>
                        <td>{{$tic->department}}</td>
                        <!-- <td>{{$tic->subject}}</td> -->
                        <td>{{$tic->description}}. @if(isset($tic->serial_number))<a href="{{env('APP_URL')}}adper/ass_ass?sn={{$tic->serial_number}}"> ({{$tic->serial_number}})</a>@endif</td>
...
function saveRow(selectElement, id) {
    ...
    let responseValue = row.querySelector('input[name="response"]').value;
        $.get('{{env('APP_URL')}}adper/ticketstatus?id='+id+'&status='+selectedValue+'&response='+responseValue, function(data, status){
    });
}
</script>