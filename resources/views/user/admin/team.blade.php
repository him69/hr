    ...
    <div class="app-main">
                    ...
                    <div class="mx-3">
                        <table class="table table-bordered text-center">
                            ...
                            <tr class="userinfocopy">
                                @csrf
                                <td><a href="{{env('APP_URL')}}adper/attendance/full_report/{{$team->id}}">{{$team->user_id}}</a> </td>
                                <td>{{$team->name}}</td>
        ...
        <script>
            function update_leader1(e,a) {
                const depId = e.value;
                fetch(`{{env('APP_URL')}}adper/getleaderbydiparment/${depId}`).then(res => res.json()).then(data => {
        ...
        <script>
            function began(leader_id, dep_id, leader_name) {
                ...
                fetch(`{{env('APP_URL')}}adper/getuserbyleader/${dep_id}/${leader_id}`).then(res => res.json()).then(data => {
                    // console.log(44);
